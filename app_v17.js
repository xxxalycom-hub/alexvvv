...
async function loadFromCloud() {
    ...
    try {
        ...
        for (const key of keysToSync) {
            ...
            // Check property (v8 compat) vs method (v9 modular/compact wrapper)
            const exists = typeof doc.exists === 'function' ? doc.exists() : doc.exists;
            
            if (exists) {
                const cloudData = JSON.parse(doc.data().content);
                // Si la nube tiene más datos o igual, confíamos en la nube
                if (cloudData.length > localData.length) {
                    localStorage.setItem(key, JSON.stringify(cloudData));
                    changedLocal = true;
                } else if (localData.length > cloudData.length) {
                    // Si el local tiene más datos, forzamos sincronización a la nube
                    syncToCloud(key, localData);
                } else {
                    // Mismo tamaño, podemos confiar en local por ahora y no sobrescribir, o asimilar nube si local está vacío
                ...
                // Existe en local pero no en nube, subir a la nube
                syncToCloud(key, localData);
            }
        }
...
function initFirebase() {
  ...
  try {
    ...
    // SECONDARY APP PARA CREAR USUARIOS SIN PERDER SESIÓN
    ...
    auth.onAuthStateChanged(user => {
      ...
      if (user) {
        ...
        db.collection('profiles').doc(user.uid).get().then(doc => {
          ...
          if (!profileData && user.email === 'admin@roseapp.com') {
            profileData = { 
              role: 'admin', 
              sede: 'ALL', 
              email: user.email,
            ...
            profileData = {
              ...
              sede: 'SEDE A', 
              email: user.email,
              access: { master: true, registry: true, reports: true, settings: false },
          ...
          refreshAllViews(); // Make sure data renders even if cloud is identical or empty
          loadFromCloud();
        ...
        console.log("Sin sesión activa. Bloqueando app.");
...
function handleLogin(e) {
  ...
  if (email.toLowerCase() === 'admin') email = 'admin@roseapp.com';
  else if (!email.includes('@')) email += '@roseapp.com';

  if (submitBtn) {
    submitBtn.disabled = true;
    submitBtn.dataset.original = submitBtn.innerHTML;
    submitBtn.innerHTML = '<i class="ph ph-circle-notch animate-spin"></i> ENTRANDO...';
  }
  ...
  auth.signInWithEmailAndPassword(email, password)
    ...
    .catch(error => {
      // Auto-prov. Admin
      if (error.code === 'auth/user-not-found' && email === 'admin@roseapp.com' && password === 'admin') {
...
function openUsersManagement() {
  document.getElementById('usersManagementModal').style.display = 'flex';
  loadAdminUsersList();
}

function loadAdminUsersList() {
  const container = document.getElementById('admin-users-list');
  container.innerHTML = '<p style="font-size: 0.85rem; color: var(--text-dim); text-align: center;">Cargando usuarios...</p>';
  
  db.collection('profiles').get().then(snapshot => {
    container.innerHTML = '';
    if (snapshot.empty) {
      container.innerHTML = '<p class="empty-msg">No hay usuarios registrados</p>';
      return;
    }
    ...
    snapshot.forEach(doc => {
      ...
      div.style.cssText = "background: rgba(255,255,255,0.05); padding: 12px; border-radius: 8px; border-left: 3px solid " + (p.role === 'admin' ? "var(--accent-cyan)" : "var(--border-color)");
      
      let accessStr = [];
      if (p.access?.master) accessStr.push("BD");
      if (p.access?.registry) accessStr.push("Reg");
      if (p.access?.reports) accessStr.push("Inv");
      if (p.access?.settings) accessStr.push("Aju");
      
      div.innerHTML = `
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:5px;">
          <strong style="font-size:0.95rem;">${p.email}</strong>
          <span style="font-size:0.75rem; background:rgba(255,255,255,0.1); padding:2px 8px; border-radius:12px;">${p.role.toUpperCase()}</span>
        </div>
        <div style="font-size:0.8rem; color:var(--text-dim); display:flex; flex-direction:column; gap:3px;">
          <span><i class="ph ph-map-pin"></i> <b>Sede:</b> ${p.sede}</span>
          <span><i class="ph ph-key"></i> <b>CRU:</b> ${p.permissions}</span>
          <span><i class="ph ph-app-window"></i> <b>Vistas:</b> ${accessStr.join(', ') || 'Ninguna'}</span>
        </div>
      `;
    ...
    container.innerHTML = '<p style="color:var(--accent);">Error al cargar.</p>';
  });
}
...
function handleCreateUser(e) {
  ...
  const btnSubmit = document.getElementById('btn-submit-create-user');
  const errorMsg = document.getElementById('create-user-error');
  
  let usr = document.getElementById('new-user-username').value.trim();
  const pwd = document.getElementById('new-user-password').value;
  const sede = document.getElementById('new-user-sede').value;
  const level = document.getElementById('new-user-level').value;
  
  const accMaster = document.getElementById('perm-master').checked;
  const accReg = document.getElementById('perm-registry').checked;
  const accRep = document.getElementById('perm-reports').checked;
  const accSet = document.getElementById('perm-settings').checked;
  ...
  btnSubmit.innerHTML = '<i class="ph ph-circle-notch animate-spin"></i> Guardando...';
  errorMsg.style.display = 'none';

  // Usar App Secundaria
  const secondaryAuth = firebase.app("Secondary").auth();
  
  secondaryAuth.createUserWithEmailAndPassword(usr, pwd)
    .then((userCred) => {
      ...
      return db.collection('profiles').doc(uid).set(newProfile).then(() => {
        // Log out the secondary user so it doesn't linger
    ...
    .then(() => {
      ...
      loadAdminUsersList(); // Refrescar lista
    })
    .catch(err => {
    ...
    .finally(() => {
      ...
      btnSubmit.innerHTML = '<i class="ph ph-plus"></i> Guardar Usuario';
    });
}
